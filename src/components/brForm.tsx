...
export function BonReceptionForm() {
  ...
  const [FournisseurData, setFournisseurData] = useState<FournisseurData[]>([]);
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    try {
      ...
      const bonReceptionData = {
        ...
        numeroPiece: data.numeroPiece, // Utiliser le numero piece à jointe
        fournisseur: data.fournisseur, // Ajouter l'ID du fournisseur
  ...
  const fetchFournisseurData = async () => {
    try {
      ...
      const formatedData: FournisseurData[] = data.map((fournisseur) => ({
          Id: fournisseur._id,
      ...
      setFournisseurData(formatedData);
    } catch (error) {
      console.error(
        "Une erreur s'est produite lors de la récupération des données des fournisseur :",
        error
      );
    }
  };
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value
                            ? FournisseurData.find(
                                (fournisseur) => fournisseur.Id === field.value
                              )?.Nom
                    ...
                    <PopoverContent className="w-[200px] p-0">
                      <Command>
                        <CommandInput placeholder="Recherche fournisseur..." />
                        <CommandEmpty>Aucun Fournisseur trouvé.</CommandEmpty>
                        <CommandGroup>
                          {FournisseurData.map((fournisseur) => (
                            <CommandItem
                              value={`${fournisseur.idFournisseur}-${fournisseur.Nom}`}
                              key={fournisseur.Id}
                              onSelect={() => {
                                form.setValue("fournisseur", fournisseur.Id);
                            ...
                            >
                              <Check
                                className={cn(
                                  "mr-2 h-4 w-4",
                                  fournisseur.Id === field.value
                                    ? "opacity-100"
                                    : "opacity-0"
                                )}
                              />
                              {`${fournisseur.idFournisseur} - ${fournisseur.Nom}`}
                            </CommandItem>
                          ))}
                        </CommandGroup>
                      </Command>
...
/>
          ...
          <div className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>pièce a joindre :</FormLabel>
                    <Input
                      {...field}
