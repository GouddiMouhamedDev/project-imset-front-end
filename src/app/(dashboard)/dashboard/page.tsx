...
export default function Dash() {
  ...
  const fetchDataAfterAuth = async () => {
    ...
    if (isAuthenticated) {
      await Promise.all([fetchProductsData(),fetchClientData(),fetchBonLivraisonData()]);
    } else {
      removeStorage();
      router.push("/login");
    }
  ...
  return (
       ...
       <> 
        ...
        <CardDataStats title="Total views" total="3.456 DT" rate="0.43%" levelUp>
          <svg
        ...
        <CardDataStats title="Bénéfice total" total="45,2 DT" rate="4.35%" levelUp>
          <svg
            className="fill-primary dark:fill-white"
            width="20"
            height="22"
        ...
        <CardDataStats title="Total Clients" total={totalClients} rate="0.95%" levelDown>
